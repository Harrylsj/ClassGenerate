            ...
            try
            {
                ...
                switch (da.DbConnectType)
                {
                    case DBConnectType.ORACLE:
                        ...
                        break;
                    case DBConnectType.SQLSERVER:
                        sql = "select TableName,IsDeleted From dbo.TableName where IsDeleted=0 order by tablename";
                        break;
                }
                ret = da.ReturnDataTable(sql);
            }
            ...
            try
            {
                switch (da.DbConnectType)
                {
                    case DBConnectType.ORACLE:
                        ...
                        ret.Tables.Add(da.ReturnDataSet("SELECT  USER_TAB_COLS.COLUMN_NAME, USER_TAB_COLS.DATA_TYPE as TYPE, USER_TAB_COLS.DATA_LENGTH as LENGTH, USER_TAB_COLS.DATA_Precision as NumLENGTH,Data_default FROM USER_TAB_COLS inner join user_col_comments on user_col_comments.TABLE_NAME=USER_TAB_COLS.TABLE_NAME and user_col_comments.COLUMN_NAME=USER_TAB_COLS.COLUMN_NAME where USER_TAB_COLS.TABLE_NAME = '" + tableName + "'").Tables[0].Copy());
            ...
            try
            {
                switch (da.DbConnectType)
                {
                    case DBConnectType.ORACLE:
                        ...
                        break;
                    case DBConnectType.SQLSERVER://20210225--lsj
//                        string sql = @"
...
//                        }
        ...
        public List<string> GetTables(ListView list)
        {
            ...
            {
                ret.Add(list.Items[i].SubItems[1].Text.Trim());
            }
            return ret;
        }
        #endregion
